  ...
  implements SpreadsheetReader
{
  ...
  private void readHeaderColsDefault()
  {
    ...
    if (colCount <= 0)
    {
      LogMgr.logError(new CallerInfo(){}, "Cannot retrieve column names because no columns are available in the first row of the sheet: " + worksheet.getTableName(), null);
  ...
  public List<Object> getRowValues(int row)
  {
    ...
    for (int col=0; col < colCount; col++)
    {
      ...
      else if ("time".equals(type))
      {
        ...
        {
          LogMgr.logWarning(new CallerInfo(){}, "Could not parse time value: " + text, ex);
          value = getTime(cell);
        }
      }
  ...
  private Object getDate(Cell cell)
  {
    ...
    {
      LogMgr.logWarning(new CallerInfo(){}, "Could not parse date format: " + fmt, ex);
      return getDateValue(cell, fmt);
    }
  }
    ...
    throws IOException
  {
    try
    {
      ...
      initCurrentWorksheet();
      LogMgr.logDebug(new CallerInfo(){}, "Document loaded. Rows: " + getRowCount());
    }
    catch (Exception ex)
    {
